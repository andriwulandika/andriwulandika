...
| R.9 — Hero collage + fullscreen nav overlay (fidelity pass 2, atas permintaan Andri "gunakan studiomodular.be sebagai template mentah") | ✅ Selesai (22 Jul 2026), menunggu review preview Andri | Dibangun ulang dengan meniru mekanisme animasi asli studiomodular.be (diteliti dari HTML/CSS/JS situs mereka via GSAP+ScrollTrigger — kami reimplementasi versi vanilla-JS/CSS-nya, tanpa menambah dependency GSAP agar tetap CSP-safe & ringan sesuai R.3/R.5): (1) headline dua baris slide-in dari kiri/kanan saat load; (2) dua foto contoh template (`template-umkm.webp`, `template-pemerintah.webp`) mengambang menembus huruf judul dengan clip-path reveal-on-load + parallax scroll, meniru collage foto khas studiomodular; (3) badge lingkaran "Scroll ke bawah" berputar di bawah hero; (4) menu mobile diganti total dari dropdown jadi overlay layar-penuh dengan animasi circle-wipe (`clip-path: circle()`) + focus-trap `inert`, meniru `main.js` overlay mereka. Font/warna brand tetap dipertahankan (tidak meniru font PolySans berbayar atau palet hijau/biru mereka — hanya mekanisme & struktur animasi). |
| R.10 — Wordmark "Andri Wulandika" raksasa menggantikan headline sebagai elemen dominan (atas jawaban eksplisit Andri: warna tetap terracotta, elemen raksasa = wordmark nama, foto collage tetap screenshot template untuk sementara) | ✅ Selesai (22 Jul 2026), menunggu review preview Andri | Ditemukan (via inspeksi HTML/CSS studiomodular.be langsung) bahwa elemen "raksasa" di referensi sebenarnya adalah SVG logo/wordmark mereka sendiri diregangkan selebar viewport — bukan kalimat headline yang diperbesar. Diterapkan padanannya: `<p aria-hidden="true">` dekoratif "Andri Wulandika" (sans Jakarta 800, `clamp(3.4rem,15vw,9.5rem)`, dua baris slide-in kiri/kanan) jadi elemen visual dominan hero, sementara kalimat value-prop asli ("Transformasi digital untuk pemerintah & bisnis.") tetap jadi `<h1>` semantik (demi SEO) tapi ukurannya diperkecil jadi teks pendukung di bawahnya. Belum ditambahkan: screenshot `ai.andriwulandika.uk` untuk collage foto — menunggu Andri kirim screenshot (headless browser sesi ini gagal fetch domain tsb, koneksi direset khusus untuk klien browser meski `curl` berhasil). |
| R.11 — Fix bug crop foto + samakan animasi & tipografi ke seluruh homepage (atas laporan bug Andri + instruksi "samakan jenis font, struktur website semua, warna saja dipertahankan") | ✅ Selesai (22 Jul 2026), menunggu review preview Andri | **Bug fix**: serpihan oranye di foto `template-umkm.webp` (tombol "Pesan Sekarang" pada screenshot sumber ikut ter-crop di tepi) — diperbaiki dengan `object-position:center 78%` supaya area navbar sumber tidak ikut kepotong di pinggir. **Animasi**: slide wordmark "Andri"/"Wulandika" diubah dari nudge kecil (3vw, staggered, fade) jadi slide penuh 100% lebar dari kiri/kanan secara bersamaan (bukan staggered), durasi 1.5s `cubic-bezier(0.25,1,0.5,1)` — meniru persis timing `xPercent -100→0` / `power4.out` GSAP di referensi. **Tipografi**: seluruh heading & elemen display (h1/h2/h3, judul section, angka harga, nomor proses, marquee, brand di header/footer) diseragamkan dari campuran Playfair-serif+Jakarta-sans menjadi sans (Jakarta) saja — meniru sistem tipografi monoton sans milik studiomodular.be. Ukuran judul section (`sec-head h2`, CTA band) diperbesar mendekati skala referensi. Warna terracotta tidak diubah. |
